...
def calcHourAvailability(m, d, h):
    total = 0
    x=random.gauss
...
def runner():
    ...
    h=0
    while (m<12):
        #print("cycling month {0}" .format(m))
        while (d<30):
            #print("cycling day {0}" .format(d))
            while (h<24):
                #print("cycling hour {0}" .format(h))
                #Catching dates that can't exist
                try:
                    dateCheck = datetime.datetime((2025), (1+m), (1+d), (h), tzinfo=pytz.UTC)
                    print("saving {0} {1}..." .format(datetime.datetime((2025), (1+m), (1+d), (h)), calcHourAvailability(m,d,h)))
                    HourAvailability.createHourAvailability((datetime.datetime((2025), (1+m), (1+d), (h), tzinfo=pytz.UTC)), (calcHourAvailability(m,d,h)))
                    print("saved")
                    h+=1
                except:
